...
def monitor_market_file(source, pullCount, saveLocation, date):
  ...
  for i, row in symbols.iterrows():
      if i <= (pullCount - 1):
        ...
        #Problem checks 
        problems = ""
        if (comp_stats[0][3] == "N/A" or str(comp_stats[0][3])[-4:] == "99.0"):
          problems = problems + "No Div Per Share, "
        if (comp_stats[0][4] == "N/A" or str(comp_stats[0][4])[-4:] == "99.0"):
          problems = problems + "No EPS, "
        if (comp_stats[0][5] == "N/A" or str(comp_stats[0][5])[-4:] == "99.0"):
          problems = problems + "No PE, "
        if (comp_stats[0][6] == "N/A" or str(comp_stats[0][6])[-4:] == "99.0"):
          problems = problems + "No Last Trade Price, "
        if (comp_stats[0][9] == "N/A" or str(comp_stats[0][9])[-4:] == "99.0"):
          problems = problems + "No Beta, "
        if (comp_stats[0][11] == "N/A" or str(comp_stats[0][11])[-4:] == "99.0"):
          problems = problems + "No Dividends, "
          comp_stats[0][11] = 0 #No dividends is possible, will have 0 div growth rate in model
        if (comp_stats[0][12] == "N/A" or str(comp_stats[0][12])[-4:] == "99.0"):
          problems = problems + "No EPS Growth, "
        if (comp_stats[0][14] == "N/A" or str(comp_stats[0][14])[-4:] == "99.0"):
...
def getDividendGrowth(ID):
  ...
  while (i < len(table)):
    ...
    if ("Stock Split" not in str(table[i])):
      ...
      if (string.find("</strong>") >= 1): 
        ...
        if (is_number(temp) and temp > 0 and is_number(string) and string != 0.0):
  ...
  if (len(dividends) > 1):
    ...
    return(float(recent_div * 1), float((sum(dividendGrowths) / len(dividendGrowths) - 1) * 4 * adjustMult))
  else:
    return 99, 99
...
def getEPSGrowthRate(ID):
  ...
  if (len(values) != 0 and epsExists == 1) and checkValues[101] == "EPS Actual":
    EPS = float(values[51]) + float(values[50]) + float(values[49]) + float(values[48])
    earningsGrowth = float(str(values2[16].replace("%",""))*1)/100
  else:
    EPS = 99
    earningsGrowth = 99

  #This is the next year growth rate
  return (float(EPS), float(earningsGrowth))

#https://finance.yahoo.com/quote/AAPL?p=AAPL
def getPE_lastClose(ID):
  #Grabs html tree, seaches for identifer of earnings table, grows next year est growth
  print("    Scraping PE, Last close price, Forward Dividend Yield ")
  #Get the PE
  page = requests.get('https://finance.yahoo.com/quote/' + ID + '?p=' + ID)
  tree = html.fromstring(page.content)
  values = tree.xpath('//span[@class="Trsdu(0.3s) "]/text()')
  values2 = tree.xpath('//td[@class="Ta(end) Fw(b) Lh(14px)"]/text()')

  #Somepages like BHI 
  if len(values) != 0 and values[0] != "N/A" and values[6] != "N/A":
    return (float(values[6]),float(values[0].replace(",", "")), str(values2[2]))
  else:
    return (99,99,99)
...
def getBeta(ID):
  ...
  page = requests.get('https://finance.google.com/finance?q=' + ID)
  tree = html.fromstring(page.content)
